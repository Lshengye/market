...
var kwords = window.localStorage.getItem('kword');
$('#head input').val(kwords);
// console.log(kwords);
var type = window.localStorage.getItem('type');
var param = window.localStorage.getItem('param');
var price1 = window.localStorage.getItem('price1');
var price2 = window.localStorage.getItem('price2');
var cid = window.localStorage.getItem('cid');

window.localStorage.setItem('price1', $('#menus .min').val())
window.localStorage.setItem('price2', $('#menus .max').val())

// 搜索结果
function Refresh(kds, ty, par, pce1, pce2, cd) {
    axios({
        method: 'GET',
        url: '//vueshop.glbuys.com/api/home/goods/search?kwords=' + kds + '&otype=' + ty + '&param=' + par + '&price1=' + pce1 + '&price2=' + pce2 + '&cid' + cd + '&token=1ec949a15fb709370f'
    }).then((res) => {
        console.log(res)
        
            $('#wares').html(res.data.data.map((v) => {
            return `<ul>
        ...
        var off = true;
        $('#sort p span').eq(0).click(function () {
            if (off) {
        ...
        $('#sort ul li').click(function () {
            if ($(this).index() == 1) {
                type ='up'
                window.localStorage.setItem('type', 'up');
            } else if ($(this).index() == 2) {
                type = 'down'
                window.localStorage.setItem('type', 'down');
            } else {
                type = 'all'
        ...
        console.log(type)
        $('#sort p span').eq(1).click(function () {
            $('#sort p span').eq(1).addClass('active')
            $('#sort ul li').removeClass('active')//清除综合选中
            type = 'sales'
            Refresh(kwords, type, param, price1, price2, cid);
            window.localStorage.setItem('type', 'sales');
        })

        // 筛选底部
        $('#menus .bottom p span').html($('#wares ul').length)
        $('#menus .bottom p').eq(1).click(function () {
...
$('#menus .bottom p').eq(2).click(function () {
    $('#screen').fadeOut(300);
    $('#menus').animate(
        { 'right': '-80%' },
        300
...
axios.get('http://vueshop.glbuys.com/api/home/category/menu?token=1ec949a15fb709370f ').then((res) => {
    ...
    for (let i = 0; i < res.data.data.length; i++) {
        document.querySelector('#menus .category ul').innerHTML += `<li id="${res.data.data[i].cid}">${res.data.data[i].title}</li>`
        // console.log($('#menus .category ul li').eq(i).attr('id'))
    }
...
axios.get('http://vueshop.glbuys.com/api/home/goods/param?kwords=' + kwords + '&token=1ec949a15fb709370f').then((res) => {
    console.log(res)
    if (res.data.data.length == 1 && res.data.data.title == "品牌") {
        ...
        for (let i = 0; i < res.data.data.param.length; i++) {
            document.querySelector('#brand-hot .brand ul').innerHTML += `<li id="${res.data.data.param[i].pid}">${res.data.data.param[i].title}</li>`;
            $('#brand-hot .hot').hide();
            if ($('#brand-hot .brand ul li').hasClass('bgc')) {
                // window.localStorage.setItem('param', $(this).attr('id'))
                // param=[$(this).attr('id')]
            }
        }
        ...
        for (let i = 0; i < res.data.data.param.length; i++) {
            document.querySelector('#brand-hot .hot ul').innerHTML += `<li id="${res.data.data.param[i].pid}">${res.data.data.param[i].title}</li>`;
        ...
        for (let i = 0; i < res.data.data[0].param.length; i++) {
            document.querySelector('#brand-hot .brand ul').innerHTML += `<li id="${res.data.data[0].param[i].pid}">${res.data.data[0].param[i].title}</li>`;
        }
        for (let i = 0; i < res.data.data[1].param.length; i++) {
            document.querySelector('#brand-hot .hot ul').innerHTML += `<li id="${res.data.data[1].param[i].pid}">${res.data.data[1].param[i].title}</li>`;
        }
        if ($('#brand-hot .brand ul li').hasClass('bgc') && $('#brand-hot .hot ul li').hasClass('bgc')) {
            window.localStorage.setItem('param', $('#brand-hot .brand ul li').eq(this).attr('id') + $('#brand-hot .hot ul li').eq(this).attr('id'))
        }
        // param=[a,b];
    }
